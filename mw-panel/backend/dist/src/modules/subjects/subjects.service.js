...
let SubjectsService = class SubjectsService {
    ...
    async findSubjectsByStudent(studentId) {
        ...
        const assignments = await this.assignmentRepository.find({
            where: { classGroup: { id: (0, typeorm_2.In)(classGroups.map(group => group.id)) } },
            relations: ['subject', 'subject.course']
        });
        const subjects = Array.from(new Map(assignments.map(assignment => [assignment.subject.id, assignment.subject])).values());
        return subjects;
    }
    async findSubjectsByTeacherAndGroup(teacherId, classGroupId) {
        const assignments = await this.assignmentRepository.find({
            where: {
                teacher: { id: teacherId },
                classGroup: { id: classGroupId }
    ...
    async findAssignmentDetails(teacherId, subjectId, classGroupId) {
    ...
    async getSubjectStatistics() {
        const [totalSubjects, totalAssignments, subjectsWithoutAssignments, teachersWithAssignments] = await Promise.all([
    ...
    async canTeacherEvaluateSubject(teacherId, subjectId, studentId) {
        ...
        const assignment = await this.assignmentRepository.findOne({
            where: {
                teacher: { id: teacherId },
                subject: { id: subjectId },
                classGroup: { id: (0, typeorm_2.In)(studentGroups.map(group => group.id)) }
            }
        });
        return !!assignment;
    }
